...
class saw
{
    ...
    {
        $data = array();
        $queryAlternative = "SELECT supplier.nama_supplier AS nama_supplier,id_supplier FROM nilai_supplier INNER JOIN supplier USING(id_supplier) WHERE id_jenisbarang='$this->idCookie' GROUP BY id_supplier ";
        $execute = $this->getConnect()->query($queryAlternative);
        while ($row = $execute->fetch_array(MYSQLI_ASSOC)) {
            array_push($data, array("nama_supplier" => $row['nama_supplier'], "id_supplier" => $row['id_supplier']));
        }
        return $data;
    }
    ...
    {
        $queryHasil     =   "SELECT hasil.hasil AS hasil,jenis_barang.namaBarang,supplier.nama_supplier AS nama_supplier FROM hasil JOIN jenis_barang ON jenis_barang.id_jenisbarang=hasil.id_jenisbarang JOIN supplier ON supplier.id_supplier=hasil.id_supplier WHERE hasil.hasil=(SELECT MAX(hasil) FROM hasil WHERE id_jenisbarang='$this->idCookie')";
