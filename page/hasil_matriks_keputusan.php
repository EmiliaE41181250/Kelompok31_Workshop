...
/***************awal set variabel************/
    ...
    $queryAlternative="SELECT supplier.nama_supplierAS nama_supplier,id_supplier FROM nilai_supplier INNER JOIN supplier USING(id_supplier) WHERE id_jenisbarang='$cookiePilih' GROUP BY id_supplier ";
...
echo "<p><h3>Matriks Keputusan</h3></p><table class='table'><tr><th rowspan='2'>Alternative</th><th colspan='$executeQueryTabel->num_rows'>Kriteria</th></tr><tr>";
...
if ($executeGetAlternative->num_rows > 0){
    while ($dataAlternative=$executeGetAlternative->fetch_array(MYSQLI_ASSOC)){
        echo"<tr id='data'><td>$dataAlternative[nama_supplier]</td>";
        ...
        while ($dataNilai=$executeNilai->fetch_array(MYSQLI_ASSOC)){
            ...
            echo "</tr>";
            $supplierArray[$indexArray]=["nama_supplier"=>$dataAlternative['nama_supplier'],"id_supplier"=>$dataAlternative['id_supplier']];
            $indexArray++;
    }
}else{
...
echo "<p><h3>Normalisasi Matriks Keputusan</h3></p><table class='table'><tr><th rowspan='2'>Alternative</th><th colspan='$executeQueryTabel->num_rows'>Kriteria</th></tr><tr>";
...
if (!empty($supplierArray)){
    ...
    if (!empty($bobotArray)) {
        for ($j=0; $j< count($supplierArray); $j++) { 
            echo "<tr id='data'><td>".$supplierArray[$j]['nama_supplier']."</td>";
...
echo "<p><h3>Normalisasi Matriks Keputusan</h3></p> <table class='table'> <tr><th rowspan='2'>Alternative</th><th colspan='$executeQueryTabel->num_rows'>Kriteria</th><th rowspan='2'>Hasil</th></tr><tr>";
foreach ($kriteriaArray as $nama_kriteria) {
    echo "<th>$nama_kriteria</th>";
}
...
/******akhir isi table perangkingan****/
    $queryHasil="SELECT hasil.hasil AS hasil,jenis_barang.namaBarang,supplier.nama_supplierAS nama_supplierFROM hasil JOIN jenis_barang ON jenis_barang.id_jenisbarang=hasil.id_jenisbarang JOIN supplier ON supplier.id_supplier=hasil.id_supplier WHERE hasil.hasil=(SELECT MAX(hasil) FROM hasil WHERE id_jenisbarang='$cookiePilih')";
    $execute=$konek->query($queryHasil)->fetch_array(MYSQLI_ASSOC);
    echo "<p>Jadi rekomendasi pemilihan supplier <i>$execute[namaBarang]</i> jatuh pada <i>$execute[nama_supplier]</i> dengan Nilai <b>".round($execute['hasil'],3)."</b></p>";
echo "</div>";
}else{
